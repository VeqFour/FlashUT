...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch('../../controllers/admin/profile.php')
    .then(res => res.json())
    .then(data => {
  ...
  : `../../template/imgProfile/instructor.png`; 
        academicSpan.textContent = data.user.academicID;
      }
    });
    

   fetch('../../controllers/admin/getAdminStats.php')
      .then(res => res.json())
      .then(data => {
      if (data.success && data.user) {
            const user = data.user;
            document.getElementById("flashReview").innerHTML = `${user.reviewed_flashcards} <br />Flashcard Reviewed`;
            document.getElementById("flashAdd").innerHTML = `${user.added_flashcards} <br />Flashcard Added`;
  ...
  <form id="uploadForm" enctype="multipart/form-data">
    <label for="picUpload">
    <img id="profilePreview" onerror="this.src='../../template/imgProfile/instructor.png'" alt="Profile Picture" class="course-image" style="cursor: pointer;" />    </label>
    <input type="file" name="profilePic" id="picUpload" accept=".jpg,.jpeg,.png,.gif,.webp,.bmp" style="display: none;" />  </form>
          ...
          <div class="contributions">
            ...
            <div class="smallBox">
              ...
              <span id="flashReview" class="flashcardViewedtext">.. <br />Flashcard Reviewed</span>
            </div>
